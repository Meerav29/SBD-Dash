...
begin
	...
	using PlutoLinks # For interactive elemnents and @ingredients
	using GLM
	using StatsPlots
...
md""" # Small Body Dashboard """

# ╔═╡ 96c7917b-cab8-4034-b763-04db489a9660
md""" This dashboard allows users to explore and analyze characteristics of small solar system bodies such as asteroids and Near-Earht Objects using the NASA JPL Small body search engine database."""
...
md""" ### Polynomial Fit Model  """

# ╔═╡ 6e56805b-2f3e-4f59-9115-ed9dc061bd6c
md""" ### Bayesian Fit Model  """
...
md""" ### Cross Validation Attempt 
1. Simple Model"""

# ╔═╡ f3c21ee6-8147-4fd7-98bf-46ddc399bc86
md""" 
So ideally we would like to be as close to 0 as possible for RSME but a value of 0.15 is also good. It is a good measure of how well the simple linear model (in log space) generalizes across folds.

2. Polynomial Model
"""

# ╔═╡ afa7d6b2-7fe6-4d02-84a7-655b0ebf9773
md"""
Again, we want to be close to 0, but 0.15 is also good. 

And we can see that both of these models - simple and polynomial are nearly equivalent in predictive accuracy. This could be an indicator that the relationship between $H$ and $log_{10}$(diameter) is nearly linear.
"""

# ╔═╡ 863b3ea3-c869-4cd6-8e37-a2d18500b598
md""" ### Residual Plots ### """
...
begin
	df_shuffled = df_clean[sample(1:nrow(df_clean), nrow(df_clean); replace=false), :]
k = 5
fold_size = Int(floor(nrow(df_shuffled) / k))

rmses = Float64[]  

for i in 1:k
    test_idx = ((i-1)*fold_size+1) : (i == k ? nrow(df_shuffled) : i*fold_size)
    train_idx = setdiff(1:nrow(df_shuffled), test_idx)
    
    train_data = df_shuffled[train_idx, :]
    test_data = df_shuffled[test_idx, :]
    
    # Fit the simple linear model on the training data
    model_cv = lm(@formula(log10(diameter) ~ H), train_data)
    
    pred_test_log = predict(model_cv, test_data)
    obs_test_log = log10.(test_data.diameter)
    
    # Calculate RMSE in log10 space
    rmse_log = sqrt(mean((obs_test_log .- pred_test_log).^2))
    push!(rmses, rmse_log)
end

mean_rmse = mean(rmses)
println("Average RMSE for simple model (log10) across $k folds: $mean_rmse")
end

# ╔═╡ f7970a47-ecca-4e7d-8faa-3f2f849970e6
begin
df_poly = deepcopy(df_clean)
df_poly.H2 = df_poly.H .^ 2

Random.seed!(1234)  
df_shuffled_poly = df_poly[sample(1:nrow(df_poly), nrow(df_poly); replace=false), :]

rmses_poly = Float64[]  # to store RMSE for each fold

for i in 1:k
    test_idx = ((i-1)*fold_size + 1):(i == k ? nrow(df_shuffled_poly) : i*fold_size)
    train_idx = setdiff(1:nrow(df_shuffled_poly), test_idx)
    train_data = df_shuffled_poly[train_idx, :]
    test_data  = df_shuffled_poly[test_idx, :]
    # Fit the polynomial model on the training data
    model_poly_cv = lm(@formula(log10(diameter) ~ H + H2), train_data)
    pred_test_log_poly = predict(model_poly_cv, test_data)
    obs_test_log_poly  = log10.(test_data.diameter)
    
    # Compute RMSE in log space
    rmse_poly_log = sqrt(mean((obs_test_log_poly .- pred_test_log_poly).^2))
    push!(rmses_poly, rmse_poly_log)
end

mean_rmse_poly = mean(rmses_poly)
...
begin 
	# using Statistics
	map(col -> count(ismissing, col), eachcol(df_subset))
end

# ╔═╡ 31767b72-c2e9-464f-989e-6d04d1124af7
begin
	# using StatsBase
	diam_mean = mean(skipmissing(df_all.diameter))
	diam_std  = std(skipmissing(df_all.diameter))
	Diam_mean_std = [mean(skipmissing(df_all.diameter)), std(skipmissing(df_all.diameter))]
end
...
histogram(residuals_simple,
    xlabel="Residual (log10 diameter)",
    title="Residual Histogram (Simple Model)",
    label="", nbins=20, alpha=0.7)

# ╔═╡ 890cb7f2-bc95-4c1e-b317-75d577beead3
model_poly = lm(@formula(log10(diameter) ~ H + (H^2)), filtered_data)
...
begin
	H_data = df_clean.H
log_diameters = log10.(df_clean.diameter)

# Define the Bayesian model
@model function bayesian_linear_model(H, log_diameters)
    # Priors for parameters
    β0 ~ Normal(0, 10)
    β1 ~ Normal(0, 10)
    σ ~ Exponential(1)

    # Likelihood: for each observation, model log10(diameter)
    for i in 1:length(H)
        log_diameters[i] ~ Normal(β0 + β1 * H[i], σ)
    end
end

# Sample from the posterior using the NUTS sampler
chain = sample(bayesian_linear_model(H_data, log_diameters), NUTS(), 1000)
end
